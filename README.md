...
- 高密度内容使用限高滚动与展开/收起；检索阶段网站可按豆包、千问、文心一言、DeepSeek、元宝 Tab 分别查看
- 同一组提示词保持跨平台统一，测试结果可按 AI 平台 Tab 切换；空数据使用“—/等待基线”而不是误导性的 0%
- AI 反推会结合历史品牌命中率、来源相关度和平台实际搜索词；未配置模型时使用可解释规则生成实验候选
- 五个平台启用/停用与登录页面快捷入口
- 回答中的品牌命中、提及次数、近似榜单位次分析
- 每条回答中的其他品牌、提及次数、推荐倾向与优先顺序分析；支持按提示词比较目标品牌和竞争品牌
...
同一个 OpenAI 配置也可用于“每条回答分析其他品牌”。该功能默认开启，采集成功后会采用混合抽取：

- 规则负责目标品牌命中、明确编号列表和可验证的出现顺序。
- AI 负责中英文品牌归一、非编号语境中的优先级、正负倾向及是否属于明确推荐。
- AI 未配置、被关闭或调用失败时自动回退到规则结果，不影响原回答和来源入库。
- 每条品牌结果保留抽取方式、置信度和原文证据；汇总页计算回答覆盖率、平均优先位次、首位率、推荐率和综合曝光分。

综合曝光分用于同一批提示词和平台样本内的相对比较，并不是市场份额：回答覆盖占 55%，回答内优先顺序占 25%，明确推荐占 20%。切换提示词后会重新按该问题的有效回答计算竞争位次。

竞争品牌覆盖率只使用已经完成品牌抽取的回答作为分母，旧版本中尚未分析的历史回答不会被当作“竞品未出现”。召回、选材和引用也按独立证据阶段统计：平台只返回最终引用时，系统会显示“仅最终引用可见”，不会补造召回过程或计算虚假转化率。
